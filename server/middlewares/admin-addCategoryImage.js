  ...
  file: (req, file) => {
    return new Promise((resolve, reject, next) => {
      crypto.randomBytes(16, async (err, buff) => {
        ...
        categoryDetails.restaurantId = req.params.id;
        
        if(categoryDetails.description == 'null') {
          categoryDetails.description = JSON.parse(categoryDetails.description);
        }

        await menuService.addCategory(categoryDetails, req.params.id).then(catResp => {
